...
async def receive_audio(websocket, output_stream, input_stream, mic_enabled_event: asyncio.Event, awaiting_response: asyncio.Event, delay_buffer: MonoDelayBuffer):
        ...
        if "delta" in response_data:
            if response_data["type"] == "response.audio.delta":
                ...
                if base64_audio_response:
                    ...
                    # 原音 + 10ms遅延のモノラル合成に変換
                    pcm16_mixed = delay_buffer.process_int16_bytes(pcm16_audio)
                    await loop.run_in_executor(None, output_stream.write, pcm16_mixed)
                    if not assistant_speaking:
                        # 念のためここでもマイクを停止
                        try:
...
async def stream_audio_and_receive_response():
    ...
    async with websockets.connect(WS_URL, additional_headers=HEADERS) as websocket:
        ...
        # 10ms遅延ミックス用バッファ
        delay_buffer = MonoDelayBuffer(samplerate=RATE, delay_ms=10)
        ...
        try:
            ...
            receive_task = asyncio.create_task(receive_audio(websocket, output_stream, stream, mic_enabled_event, awaiting_response, delay_buffer))
